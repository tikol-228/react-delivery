...
// if a build of the React app exists, serve it from Express
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const staticPath = path.join(__dirname, "../../react-delivery/dist");
// Express will gracefully ignore requests when the directory doesn't exist,
// so we can mount static middleware unconditionally.  This allows the same
// server command to work in both dev and production once the client has been
// built (see "build:client" script).  In dev use the "start:all" helper
// which builds the client before launching the server.
